...
function userMoves() {
    if (symCheck) {
        ...
        const trackTurnFunc = () => {
            ...
            const allTracker = document.querySelectorAll('.tracker')
            const allTrackerArray = Array.from(allTracker)
            guiCountTrack.setAttribute('class', 'tracker')
            document.body.appendChild(guiCountTrack)
            if (turnCounter % 2 == 0) {
                guiCountTrack.innerHTML = 'Player 1 Turn'
                console.log(allTrackerArray);
                allTrackerArray[turnCounter - 1].innerHTML = ""
            } else {
